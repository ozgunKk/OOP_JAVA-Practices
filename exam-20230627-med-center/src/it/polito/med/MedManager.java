...
public class MedManager {
	...
	private Map<String, Doctor> doctors = new HashMap<>();        // Map<Doctor id, Doctor>
	private Map<String, Schedule> schedules = new HashMap<>();    // Map<Doctor id, Schedules>
	private Map<String, Appointment> appointments = new HashMap<>();   //Map<SSN, Appointment>
	private Map<String, List<String>> receptionRecords = new HashMap<>();
	...
	public void addSpecialities(String... specialities) {
		for(String s: specialities) {
			if(!specialityList.contains(s)) specialityList.add(s);
		}
	...
	public void addDoctor(String id, String name, String surname, String speciality) throws MedException {
		...
		if(specialityList.contains(speciality) && (!doctors.keySet().contains(id))) {
	...
	public Collection<String> getSpecialists(String speciality) {
		return doctors.values().stream()
				               .filter(x->x.getSpeciality().equals(speciality))
				               .map(Doctor::getId)
	...
	public int addDailySchedule(String code, String date, String start, String end, int duration) {
		if(schedules.keySet().contains(code)) {
			schedules.get(code).assignSlots(date, start, end, duration);
		}else
		    schedules.put(code, new Schedule(date, start, end, duration));
		
		return schedules.get(code).getNumOfSlots(date);
	}
	...
	public Map<String, List<String>> findSlots(String date, String speciality) {
		return schedules.entrySet().stream()
				                   .filter(d->doctors.values().stream()
				                		                      .filter(doc->doc.getSpeciality() == speciality)
				                		                      .map(Doctor::getId)
				                		                      .toList().contains(d.getKey())
				                           )
				                   .collect(Collectors.toMap(Map.Entry::getKey, 
				                		   s->s.getValue().getSchedulesPerDay(date)));
	...
	public String setAppointment(String ssn, String name, String surname, String code, String date, String slot) throws MedException {
		...
		if(schedules.keySet().contains(code) && schedules.get(code).checkDate(date) == true && schedules.get(code).getSchedulesPerDay(date).contains(slot)) {
			unique = code+ssn+date+slot;
			appointments.put(unique, new Appointment(unique,ssn, name, surname, code, date, new SlotHours(slot)));
			schedules.get(code).setSlotState(date, new SlotHours(slot), false);
			return unique;
		}
		else throw new MedException();
	...
	public String getAppointmentDoctor(String idAppointment) {
		return appointments.get(idAppointment).getDocId();
	}
	...
	public String getAppointmentTime(String idAppointment) {
		return appointments.get(idAppointment).getSlot().split("-")[0];
	}
	...
	public int setCurrentDate(String date) {
		currentDate = date;
		if(!receptionRecords.keySet().contains(date)){
			receptionRecords.put(date, new ArrayList<String>());
		}
	...
	public void accept(String ssn) {
		receptionRecords.get(currentDate).add(0, ssn);
		/*
        for(Appointment ap: appointments.values()) {
        	if(ap.getDate() == currentDate && ap.getSsn() == ssn) {
        		ap.setDone(); break;
        	}
        }
        */
	...
	public String nextAppointment(String code) {
	    for(Appointment ap: appointments.values()) {
	    	if(ap.getDate().equals(currentDate) && ap.getSsn().equals(receptionRecords.get(currentDate).get(0)) && ap.getDocId().equals(code)) {
        		return ap.getCode();	        	
        	}
	    }
		return null;
	...
	public void completeAppointment(String code, String appId)  throws MedException {
		if(appointments.keySet().contains(appId) && doctors.keySet().contains(code) && appointments.get(appId).getDate().equals(currentDate)) {
		    appointments.get(appId).setDone();
		    receptionRecords.get(currentDate).set(0, "done");
		}
		else
			throw new MedException();
	}
	...
	public double showRate(String code, String date) {
		int numOfAppOfDoc = listAppointments(code,date).size();
		int CompAppOfDocPerDay = (int)schedules.get(code).getSlotTable().get(date).values().stream().filter(v-> v == false).count();
		
		return CompAppOfDocPerDay/numOfAppOfDoc;
	}
